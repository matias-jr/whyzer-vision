...
const footerLinks: Record<string, { label: string; href: string; target?: string }[]> = {
  ...
  Resources: [
    { label: 'Whyzer Academy', href: 'https://subscribe.whyzer.ai/academy-registration', target: '_blank' },
    { label: 'Help Center', href: 'http://intercom.help/whyzerai/en', target: '_blank' },
    { label: 'FAQ', href: '#faq' },
  ...
  Company: [
    ...
    { label: 'Members Area', href: 'https://members.whyzer.ai/login', target: '_top' },
  ],
};

const SUBSCRIBE_PREFIX = 'https://subscribe.whyzer.ai/';
    ...
    <footer className="border-t border-foreground/[0.06] py-16 px-6 lg:px-12 bg-background">
      <div className="max-w-[1200px] mx-auto grid grid-cols-2 md:grid-cols-4 gap-10">
        <div className="col-span-2 md:col-span-1">
          <a href="https://www.whyzer.ai" target="_top" className="inline-block mb-4">
            <img
          ...
          <div className="flex gap-3">
            <a href="https://www.linkedin.com/company/whyzer-ai" target="_blank" rel="noopener noreferrer" className="text-text-tertiary hover:text-foreground transition-colors" aria-label="LinkedIn">
          ...
          <div key={title}>
            ...
            <ul className="space-y-2.5">
              {links.map((link) => (
                <li key={link.label}>
                  <a
                    href={link.href.startsWith(SUBSCRIBE_PREFIX) ? appendUtm(link.href) : link.href}
                    target={link.target}
